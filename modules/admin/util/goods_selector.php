...
class Goods_selector_Snailfishshop extends AdminController 
{
	public function main($page = 0) 
	{
		...
		{
			$query_field = 'id,goodsname,total,hasoption,price,sales';
			$tablename = tablename('lionfish_comshop_goods');
			$condition .= ' AND type = "normal" and grounding = 1';
		}
		...
		if (!(empty($list))) 
		{
			foreach ($list as &$li ) 
			{
				$thumb = pdo_fetch('select * from ' . tablename('lionfish_comshop_goods_images') . ' where goods_id=:goods_id and uniacid=:uniacid order by id asc Limit 1', array( ':uniacid' => $_W['uniacid'], ':goods_id' => $li['id']));
				$li['thumb'] = tomedia($thumb['thumb']);
			}
		}
		...
		{
			include $this->display('util/goods_selector');
		}
	}
	public function op() 
	{
		...
		include $this->display('util/goods_selector_op');
	}
	public function js() 
	{
		global $_W;
		include $this->display('util/goods_selector_js');
	}
	public function getcate() 
	{
		$category = load_model_class('goods_category')->getAllCategory();
