...
class Order_Snailfishshop extends MobileController
{
	...
	**/
	...
	public function order_info()
	{
		...
		$is_supply = isset($_GPC['is_supply']) ? $_GPC['is_supply'] :'';
		
		if( $is_supply > 0 )
		{
			$supply_is_open_mobilemanage = load_model_class('front')->get_config_by_name('supply_is_open_mobilemanage');
			...
			$supply_info = pdo_fetch("select * from ".tablename('lionfish_comshop_supply')." where uniacid=:uniacid and member_id=:member_id ", 
			...
			{
				$ck_order_goods = pdo_fetch("select * from ".tablename('lionfish_comshop_order_goods')." 
								where uniacid=:uniacid and supply_id= and order_id=:order_id ", 
								array(':uniacid' => $_W['uniacid'], ':supply_id' => $supply_info['id'], ':order_id' => $order_id ));
				
				if( !empty($ck_order_goods) )
				{
					
					$order_info = pdo_fetch("select * from ".tablename('lionfish_comshop_order')." 
						where uniacid=:uniacid and order_id=:order_id and member_id=:member_id ", array(':uniacid' => $_W['uniacid'], ':order_id' => $ck_order_goods['order_id']) );
	    ...
	    foreach($order_goods_list as $key => $order_goods)
	    {
		   ...
		   $order_goods_comment_info = pdo_fetch('select comment_id from '.tablename('lionfish_comshop_order_comment')." where order_id=:order_id and uniacid=:uniacid and goods_id=:goods_id ", 
			...
			$order_goods_refund_list = pdo_fetchall("select real_refund_quantity,money,addtime from ".tablename('lionfish_comshop_order_goods_refund')." 
					where uniacid=:uniacid and order_goods_id=:order_goods_id order by id asc ", 
	...
	public function orderlist()
	{
		...
		$is_supply = isset($_GPC['is_supply']) ? $_GPC['is_supply'] :0;
		...
		else if( $is_supply == 1 )
		{
			...
			if( !empty($supply_info) &&  $supply_info['state'] == 1 && $supply_info['type'] == 1 && $supply_info['is_open_mobilemanage'] == 1 )
			{
				...
				if( !empty($keyword) )
				{
					$left_join = ' inner join ( select DISTINCT(og.order_id) from ' . tablename('lionfish_comshop_order_goods') . ' as og  where  (locate("'.$keyword.'",og.name)>0) and og.supply_id = '.$supply_info['id'].' ) gs on gs.order_id=o.order_id';
				}else{
					$left_join = ' inner join ( select DISTINCT(og.order_id) from ' . tablename('lionfish_comshop_order_goods') . ' as og  where   og.supply_id = '.$supply_info['id'].' ) gs on gs.order_id=o.order_id';
				}
	    ...
	    $sql = "select o.order_id,o.order_num_alias,o.date_added,o.delivery,o.is_pin,{$fields} o.is_zhuli,o.shipping_fare,o.shipping_address,o.shipping_tel,o.shipping_name,o.voucher_credit,o.fullreduction_money,o.store_id,o.total,o.order_status_id,o.lottery_win,o.type,os.name as status_name from ".tablename('lionfish_comshop_order')." as o  {$left_join}, {$join} 
	    ...
	    foreach($list as $key => $val)
	    {
	        ...
	        $goods_sql = "select order_goods_id,head_disc,has_refund_money,has_refund_quantity, member_disc,level_name,goods_id,is_pin,shipping_fare,name,goods_images,quantity,price,total,rela_goodsoption_valueid   
	            from ".tablename('lionfish_comshop_order_goods')." where uniacid=:uniacid and  order_id= ".$val['order_id']."";
	        ...
	        foreach($goods_list as $kk => $vv) 
	        {
				...
				if($is_tuanz == 1){
					...
					$order_goods_refund_list = pdo_fetchall("select * from ".tablename('lionfish_comshop_order_goods_refund')." 
												where uniacid=:uniacid and order_id=:order_id and order_goods_id=:order_goods_id ", 
												array(':uniacid' => $_W['uniacid'], ':order_id' => $val['order_id'], ':order_goods_id' => $vv['order_goods_id'] ) );
		...
		$open_aftersale = load_model_class('front')->get_config_by_name('open_aftersale');
		...
		$open_aftersale_time = load_model_class('front')->get_config_by_name('open_aftersale_time');
